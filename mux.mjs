...
// one narrated mp4 per chapter, then the whole manual with chapter markers.
//
...
// clip being sped up or the audio cut off.
//
// Run this after tour.mjs, and only once verify-redaction.mjs reports CLEAN:
...
for (const ch of manifest) {
  ...
  const mp3 = path.join(OUT, 'voice', `${ch.id}.mp3`);
  const mp4 = path.join(OUT, 'mp4', `${ch.id}.mp4`);
...
if (!made.length) {
  console.error('nothing to mux');
  process.exit(1);
}

// concat list and chapter markers, both measured from what was actually made
fs.writeFileSync(path.join(OUT, 'concat.txt'),
  made.map((m) => `file 'mp4/${path.basename(m.mp4)}'`).join('\n') + '\n');
...
fs.writeFileSync(path.join(OUT, 'chapters.txt'), meta.join('\n') + '\n');

// Every chapter came out of the same encoder, so the join is a stream copy.
const full = path.join(OUT, 'employee-management-manual.mp4');
ff(['-f', 'concat', '-safe', '0', '-i', path.join(OUT, 'concat.txt'),
  '-i', path.join(OUT, 'chapters.txt'), '-map_metadata', '1',
  '-c', 'copy', '-movflags', '+faststart', full]);

const mins = Math.floor(at / 60);
console.log(`\n${made.length} chapters -> ${path.basename(full)} `
